...
my %me2x5 = &makescorematrix("$dirname/me2x5");
my %seq = &makesequencematrix("$dirname/splicemodels/splice5sequences");
...
sub scoreconsensus{
  ...
  if ($bgd{$seqa[3]} == 0) { $bgd{$seqa[3]} = 1; }
  if ($bgd{$seqa[4]} == 0) {$bgd{$seqa[4]} = 1; }
